...
const certRegex = /(.+)\.cert(ificate)?\.pem$/;
const keyRegex = /(.+)\.private\.key$/;
const autoName = ((name) => {
  const keysDir = "keys-" + name;
...
function loadFromEnv() {
  ...
  exports.config.client_id = autoName ? CLIENT_ID + "-" + autoName : CLIENT_ID;
}

function autoDir() {
  return autoName ? "keys-" + autoName : defaultKeysDir;
}
...
async function buildConnection() {
  ...
  console.log(`Using cert ${exports.config.cert} as client ${exports.config.client_id}`);
